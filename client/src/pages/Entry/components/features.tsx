...
const Features: React.FC<EntryBannerProps> = (props) => {
  const items: featuresItem[] = [
    {
      key: 1,
      title: "Step 1: Locate a farm around you",
      icon: <EnvironmentOutlined />,
      children: <div>Find a farm near you.</div>,
    },
    {
      key: 2,
    ...
    },
    {
      ...
      children: (
        <div>
          Go to your farm and have a chat with the other farmers to check in how
          they're doing. You can do whatever to help with the community.
        </div>
      ),
    },
    {
      ...
      children: (
        <div>
          Once your farm is completed, record your observations in our
          post-visit form to highlight if the farm requires any extra attention
          or resources.
        </div>
      ),
    },
  ...
  const featureItems = items.map((item) => {
    return (
      <div className="descRow" key={item.key}>
        <div className="descIcon">{item.icon}</div>
        <div className="desc">
          <div className={classNames("descTitle", "accent")}>{item.title}</div>
          {item.children}
        </div>
      </div>
    );
  ...
  return (
    <div className={"entryContainer"} ref={props.sectionRef}>
      <div className={cn("featurebanner", "fullHeight")}>
        ...
        >
          {" "}
          <ArrowDownOutlined /> So what differentiates us?{" "}
        </a>
      </div>
